...
/*
Plugin Name: Netpluspay Pre Auth Payment Gateway
Plugin URI: https://www.netpluspay.com
Description: Netpluspay Pre Auth Payment gateway for woocommerce
Version: 0.5
Author: NetplusDotCom
Author URI: https://www.netpluspay.com

*/
add_action( 'plugins_loaded', 'woocommerce_netpluspay_pre_auth_init');

function woocommerce_netpluspay_pre_auth_init()
{
	if(!class_exists('WC_Payment_Gateway')) return;
	
	class WC_NetpluspayPreAuth  extends WC_Payment_Gateway
		{
		public function __construct()
				{
			$this->id = 'netpluspay_pre_auth';
			$this ->method_title = 'Netpluspay Pre Auth';
			...
			add_action( 'woocommerce_api_wc_netpluspaypreauth',array( $this, 'check_netpluspay_response' ));
			...
			add_action('woocommerce_receipt_netpluspay_pre_auth', array(&$this, 'receipt_page'));
		}
		function init_form_fields()
			...
			$this -> form_fields = array(
			                ...
			                'title' => array(
			                    ...
			                    'default' => __('Netpluspay  Pre Auth', 'Npp')),
			                'description' => array(
			                ...
			                'merchant_id' => array(
			                    'title' => __(' Pre Auth Merchant ID', 'Npp'),
			                    'type' => 'text',
			                    'description' => __('Your Netpluspay Merchant ID')), 
			                'test_merchant_id' => array(
			                    'title' => __(' Pre Auth Test Merchant ID', 'Npp'),
			                    'type' => 'text',
		...
		public function admin_options(){
			echo '<h3>'.__('Netpluspay  Pre Auth Payment Gateway', 'Npp').'</h3>';
		...
		* Generate payu button link
			...
			$netpluspay_args = array(
			          ...
			          'return_url' => WC()->api_request_url( 'WC_NetpluspayPreAuth' ),
            ...
            <input type="submit" class="button-alt" id="submit_payu_payment_form" value="'.__('Pay via NetPlusPay Pre Auth', 'mrova').'" /> <a class="button cancel" href="'.$order->get_cancel_order_url().'">'.__('Cancel order &amp; restore cart', 'mrova').'</a>
				</form>';
		...
		function check_netpluspay_response(){
			...
			if($status ==  "Success")
				...
				$order->add_order_note('Payment Via NetPlusPay PreAuth');
				// 				Reduce stock levels
			...
			elseif($status ==  "Payment Failed")
				...
				$order->add_order_note('Payment Via Netpluspay Pre Auth');
	...
	* Add the Gateway to WooCommerce
	     **/
	    function woocommerce_add_netpluspay_pre_auth_gateway($methods) {
		$methods[] = 'WC_NetpluspayPreAuth';
		return $methods;
	}
	
	add_filter('woocommerce_payment_gateways', 'woocommerce_add_netpluspay_pre_auth_gateway' );
}
